...
Thread pool 与 asyncio 能改善本例，是因为请求会等待。若工作变为重度纯 Python CPU 计算，受 GIL（Global Interpreter Lock）影响，普通线程通常不能线性加速，此时值得学习 process-based parallelism。
...
可运行的核心代码聚焦于 URL 规范化、去重、成功/失败状态和在途请求限制。接下来值得学习：

| 主题 | 为什么重要 |
| --- | --- |
| Timeout（超时） | 慢请求不应永久占用容量 |
| Exception propagation（异常传播） | 后台失败必须能够被观察到 |
| Cancellation（取消） | 停止抓取时，应清理未完成任务 |
| Retry with backoff（退避重试） | 临时失败需要受控恢复 |
| Rate limiting（限速） | 有并发能力不代表应压迫网站 |
| Page/depth limit（页面/深度限制） | 在途请求限制不等于限制整体抓取增长 |
| `robots.txt` 与 `User-Agent` | 真实 crawler 应负责任地运行 |
...
7. 加入一个小型可靠性扩展：timeout、失败页面 retry 或最大页面数限制。
...
英文与中文 README 需要一起维护。GitHub Actions 检查会在 pull request 只修改其中一种语言文档，或移除语言切换链接时失败。修改概念、图、命令或示例时，请在同一个 pull request 中同步更新两个文件。
